...
`remove_worktrees: true` consults no Workspace status at all. It hands every linked worktree to git forced, including one whose workflow run is still in flight, and leaves the branches behind. So it is not the way round a `remove-workspace` that was refused: the refusal is about the directory a run is writing in, and taking the whole Project removes that same directory just as surely, with the run still going in it.
...
`remove-workspace` is the `Remove` action on the [Project](projects-and-workspaces.md) screen, reached by an agent instead of by you. It takes the Workspace path and all three removal flags on every call — there are no defaults to inherit. `force_delete_worktree` removes a worktree that has local changes, which git otherwise refuses. `delete_branch` deletes the Workspace's branch once the worktree is gone, and `force_delete_branch` deletes one that is not fully merged. Leaving any of the three out is refused — `The delete branch field is required.` — because an unstated flag read as false would decide a destruction you never answered either way. Stating `force_delete_branch: true` alongside `delete_branch: false` is refused too, with `force_delete_branch requires delete_branch to be true.`, rather than silently ignored, which is what git would do with that pairing. A false `force_delete_branch` is how a branch is kept, and is accepted beside a false `delete_branch` on every call.

The same Workspaces the screen offers `Remove` for are the ones the tool accepts: `suspended`, `error` or `unknown`. The primary Workspace is the Project's own directory and can never be removed. A `ready` Workspace is refused until `override-workspace-status` sets it to `suspended`, and a Workspace whose run is still in flight is refused until the run ends — the run is executing inside the directory the removal would delete. That gate is this tool's alone: `remove-project` with `remove_worktrees: true` removes every linked worktree of the Project without reading a status, so it is no substitute for a `remove-workspace` that was refused. Nothing outside the git worktree records a Workspace, so a removal leaves nothing behind, and nothing undoes it.

The two halves of a removal do not stand or fall together. The worktree goes first and the branch second, and `git branch --delete` refuses a branch that is not fully merged — so a `delete_branch: true` call against an unmerged branch comes back as that refusal after the directory has already been removed. The error names the branch and says nothing about the worktree, which is gone all the same. Retrying is not the answer: the Workspace no longer resolves, a second call is `Workspace at path '…' not found.`, and no tool published here deletes a branch on its own. Delete it by hand in the repository, or pass `force_delete_branch: true` on the first call when you already know the branch is unmerged.
