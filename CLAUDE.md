...
- Deliberate seams are `protected` methods: `LaunchService::launchProcess()` (its `create_new_console` + faked `PendingProcess::start()` combination is fatal), `RunPendingCliCommand::navigateTo()` and `NativeAppServiceProvider::navigateTo()` (no Electron in tests). `GitService::runGit()` is private and is *not* a seam — fake the Process facade instead.
